 ...
 */
public class HanaDialectCreatorImpl extends AbstractDialectCreator {

    @Override
    public Class<?> generate(UrlDriver driver) {
        if (ComparatorUtils.equals(driver.getDriver(), "com.sap.db.jdbc.Driver")) {
    ...
    public Class<?> generate(Connection connection) {
        try {
            if (ComparatorUtils.equals(connection.getMetaData().getDriverName(), "Hana JDBC")) {
                return HanaDialect.class;
            }
        } catch (SQLException e) {
            BILoggerFactory.getLogger(HanaDialectCreatorImpl.class).error(e.getMessage(), e);
        }
        return null;
    }
}