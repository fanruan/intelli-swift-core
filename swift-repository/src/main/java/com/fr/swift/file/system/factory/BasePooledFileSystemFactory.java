...
public abstract class BasePooledFileSystemFactory<S extends SwiftFileSystem, C extends SwiftFileSystemConfig> implements SwiftFileSystemFactory<S, C> {
    protected ConcurrentHashMap<SwiftFileSystemConfig, BaseRemoteSystemPool<S>> poolContainer;

    public BasePooledFileSystemFactory() {
        poolContainer = new ConcurrentHashMap<SwiftFileSystemConfig, BaseRemoteSystemPool<S>>();
    }

    protected abstract BaseRemoteSystemPool<S> createPool(C config);
