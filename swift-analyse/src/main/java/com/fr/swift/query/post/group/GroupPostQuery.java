 ...
 */
public class GroupPostQuery extends AbstractPostQuery<NodeResultSet> {
    ...
    public NodeResultSet getQueryResult() throws SQLException {
        ...
        // 更新Node#data
        int dimensionSize = info.getDimensions().size();
        ...
        if (hasDimensionTargetSorts(info.getDimensions())) {
            NodeSorter.sort(mergeResult.getNode(), getDimensionTargetSorts(info.getDimensions()));
...
//                (GroupNode) mergeResult.getNode(), info.getTargetInfo().getResultAggregators());
    ...
    static boolean hasDimensionTargetSorts(List<Dimension> dimensions) {
    ...
    private static List<MatchFilter> getDimensionMatchFilters(List<Dimension> dimensions) {
        List<MatchFilter> matchFilters = new ArrayList<MatchFilter>(dimensions.size());
        for (Dimension dimension : dimensions) {
//            FilterInfo filter = dimension.getFilter();
     ...
     * 维度根据结果（比如聚合之后的指标）排序
     */
    static List<Sort> getDimensionTargetSorts(List<Dimension> dimensions) {
