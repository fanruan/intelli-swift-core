...
public class SwiftCommonProcessHandler extends BaseProcessHandler<List<URL>> implements CommonProcessHandler {
    ...
    public List<URL> processUrl(Target target, Object... args) {
        ...
        switch (target) {
            case ANALYSE:
                clusterIds = ClusterSwiftServerService.getInstance().getClusterEntityByService(ServiceType.ANALYSE).keySet();
                break;
            case HISTORY:
                clusterIds = ClusterSwiftServerService.getInstance().getClusterEntityByService(ServiceType.HISTORY).keySet();
                break;
            case REAL_TIME:
                clusterIds = ClusterSwiftServerService.getInstance().getClusterEntityByService(ServiceType.REAL_TIME).keySet();
                break;
            case INDEXING:
                clusterIds = ClusterSwiftServerService.getInstance().getClusterEntityByService(ServiceType.INDEXING).keySet();
                break;
            case ALL:
