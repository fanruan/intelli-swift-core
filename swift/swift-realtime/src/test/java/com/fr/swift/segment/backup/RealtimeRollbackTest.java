 ...
 */
public class RealtimeRollbackTest extends TestCase {
    ...
    public void testRollbackWithOldSeg() {
        try {
            ...
            assertTrue(segment.getAllShowIndex().contains(0));
            assertTrue(segment.getAllShowIndex().contains(41));
            Map<String, Map<Integer, ImmutableBitMap>> indexMap = new HashMap<String, Map<Integer, ImmutableBitMap>>();
            for (String columnName : segment.getMetaData().getFieldNames()) {
                indexMap.put(columnName, new HashMap<Integer, ImmutableBitMap>());
            ...
            Map<String, Map<Integer, ImmutableBitMap>> newIndexMap = new HashMap<String, Map<Integer, ImmutableBitMap>>();
            for (String columnName : segment.getMetaData().getFieldNames()) {
                newIndexMap.put(columnName, new HashMap<Integer, ImmutableBitMap>());
                Column column = segment.getColumn(new ColumnKey(columnName));
                int size = column.getDictionaryEncodedColumn().size();
                for (int i = 0; i < size; i++) {
