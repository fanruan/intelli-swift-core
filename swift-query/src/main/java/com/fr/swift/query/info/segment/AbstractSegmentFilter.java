...
public abstract class AbstractSegmentFilter implements SegmentFilter {
    ...
    private boolean isLineAllot(SingleTableQueryInfo singleTableQueryInfo) {
        return tableAllotRule == null || segmentBucket == null
    ...
    private Set<Integer> getVirtualOrder(SwiftDetailFilterInfo filterInfo, SourceKey table) throws SwiftMetaDataException {
        ...
        SwiftMetaDataColumn swiftMetaDataColumn = SwiftContext.get().getBean(SwiftMetaDataService.class).getMetaDataByKey(table.getId()).getColumn(hashFieldIndex + 1);
