...
public class BIRandomUitils {
    ...
    public static String getRandomLetterString(int length) {
        return getRandomString(length, alphabet_base);
    }
    ...
    public static class RandomGenerator {
        ...
        public String getRandomString(int length, String base) {
            StringBuffer sb = new StringBuffer();
            for (int i = 0; i < length; i++) {
                int number = random.nextInt(base.length());
                sb.append(base.charAt(number));
        ...
        public int getRandomAbsInteger() {
            return Math.abs(random.nextInt());
        }

        public String getRandomCharacterString(int length) {
            /**
             *  "啊啵额风格阿私里法噗机师的飞德洒方司壹贰叁肆伍陆柒捌玖拾佰仟万万亿元角分零整正一二两三四五六七八九十念毛另"的编码
             *  为了过代码检测中不能有中文的限制
             */
        ...
        public String getRandomLetterString(int length) {
            return getRandomString(length, alphabet_base);
        }

        public byte[] getRandomByte(Integer size) {
            byte[] bytes = new byte[size];
            random.nextBytes(bytes);
            return bytes;
        }
