 ...
 * mapping. This is not meant to be usable by end users.
 * 
...
public final class MutableRoaringArray implements Cloneable, Externalizable,
    ...
    static final short SERIAL_COOKIE = 12345;
    ...
    public void deserialize(DataInput in) throws IOException {
        this.clear();
        final byte[] buffer4 = new byte[4];final byte[] buffer = new byte[2];
        ...
        final short keys[] = new short[this.size];final int cardinalities[] = new int[this.size];
        final boolean isBitmap[] = new boolean[this.size];
        for (int k = 0; k < this.size; ++k) {
            in.readFully(buffer);
                ...
                for (int l = 0; l < bitmapArray.limit(); ++l) {
                    ...
                    bitmapArray.put(l,
                            (((long) buf[7] << 56)
                                    ...
                                    + ((long) (buf[4] & 255) << 32)
                                    + ((long) (buf[3] & 255) << 24)
                                    + ((buf[2] & 255) << 16) + ((buf[1] & 255) << 8) + (buf[0] & 255)));
                }
