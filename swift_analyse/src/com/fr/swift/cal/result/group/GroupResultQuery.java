...
public class GroupResultQuery extends AbstractGroupResultQuery {
    ...
    public GroupResultQuery(List<Query<RowResultCollector>> queries, List<Aggregator> aggregators, List<GroupTarget> targets) {
        super(queries, aggregators, targets);
    }

    public GroupResultQuery(List<Query<RowResultCollector>> queries, List<Aggregator> aggregators, List<GroupTarget> targets, List<Sort> indexSorts, List<MatchFilter> dimensionMatchFilter) {
        super(queries, aggregators, targets);
    ...
    @Override
    public RowResultCollector getQueryResult() throws SQLException {
        // 暂时不考虑对结果的过滤
        // 这边因为要构建node，需要用到cube的字典（找到全局索引对应的分组值）
        return RowResultAggregationUtils.query(queryList, aggregators, indexSorts);
    }
}
