...
//        //每天最多迁移多少个segment
        Map<Segment, SegmentKey> limit = allSegments.stream().limit(2).map(r -> new Pair<>(SegmentUtils.newSegment(r), r)).collect(Collectors.toMap(Pair::getKey, Pair::getValue));
        try {
            TaskProducer.produceTask(new MigrateExecutorTask(limit));
        } catch (Exception e) {
