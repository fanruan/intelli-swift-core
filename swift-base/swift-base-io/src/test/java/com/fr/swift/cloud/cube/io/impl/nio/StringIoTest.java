...
public class StringIoTest extends BaseIoTest {
    ...
    public void test() {
        ...
        ObjectIo<String> io = new StringNio(new NioConf(path, NioConf.IoType.APPEND, pageSize, pageSize, false));
        for (int i = 0; i < strings.length; i++) {
            io.put(i, strings[i]);
        }
        io.release();

        io = new StringNio(new NioConf(path, NioConf.IoType.APPEND, pageSize, pageSize, false));
        String third = "0987654321";
        io.put(2, third);
        io.release();

        io = new StringNio(new NioConf(path, NioConf.IoType.READ, pageSize, pageSize, false));
        for (int i = 0; i < strings.length; i++) {
