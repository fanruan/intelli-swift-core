package com.fr.bi.conf.data.source.operator.add.date;


import com.fr.bi.base.annotation.BICoreField;
import com.fr.bi.base.key.BIKey;
import com.fr.bi.common.inter.Traversal;
import com.fr.bi.conf.data.source.operator.add.AbstractAddColumnOperator;
import com.fr.bi.stable.constant.DBConstant;
import com.fr.bi.stable.constant.DateConstant;
import com.fr.bi.stable.data.db.BIDataValue;
import com.finebi.cube.api.ICubeTableService;
import com.fr.bi.stable.engine.index.key.IndexKey;
import com.fr.bi.stable.utils.code.BILogger;
import com.fr.bi.stable.utils.time.BIDateUtils;
import com.fr.json.JSONObject;
import com.fr.stable.StringUtils;
import com.fr.stable.xml.XMLPrintWriter;
import com.fr.stable.xml.XMLableReader;

/**
 * Created by GUY on 2015/3/5.
 */
public class GetValueFromDateOperator extends AbstractAddColumnOperator {
	
	

    public static final String XML_TAG = "DateDiffOperator";
    private static final long serialVersionUID = -1675716963282566541L;
    @BICoreField
    private String field;
    @BICoreField
    private int unit;
    
    public GetValueFromDateOperator(long userId) {
        super(userId);
    }

    public GetValueFromDateOperator() {
    }

    @Override
    public String xmlTag() {
        return XML_TAG;
    }

    @Override
    public JSONObject createJSON() throws Exception {
        JSONObject jo = super.createJSON();
        jo.put("field1", field);
        jo.put("unit", unit);
        return jo;
    }

    /**
     * 将JSON对象转换成java对象
     *
     * @param jsonObject json对象
     * @throws Exception 报错
     */
    @Override
    public void parseJSON(JSONObject jo) throws Exception {
        super.parseJSON(jo);
        if(jo.has("field1")){
        	field = jo.getString("field1");
        }
        if(jo.has("unit")){
        	unit = jo.getInt("unit");
        }
    }


    @Override
    protected int write(Traversal<BIDataValue> travel, ICubeTableService ti, int startCol) {
        int rowCount = ti.getRowCount();
        DateGetter dg = getDateGetter(unit);
        BIKey key = new IndexKey(field);
        BIDateUtils.checkDatefieldType(ti.getColumns(), key);
        for (int row = 0; row < rowCount; row++) {
            Object value = dg.get((Long)ti.getRow(key, row));
            try {
                travel.actionPerformed(new BIDataValue(row, startCol, value));
            } catch (Exception e) {
                BILogger.getLogger().error("incorrect formular");
                travel.actionPerformed(new BIDataValue(row, startCol, null));
            }
        }
        return rowCount;
    }

    /**
     * @param key2 
     * @param key1 
     * @param map 
	 * 
	 */


	@Override
    public void readXML(XMLableReader reader) {
        super.readXML(reader);
        if (reader.isAttr()) {
            try {
                this.field = reader.getAttrAsString("field1", StringUtils.EMPTY);
                this.unit = reader.getAttrAsInt("unit", DateConstant.DATE.DAY);
            } catch (Exception e) {
            }
        }

    }

    @Override
    public void writeXML(XMLPrintWriter writer) {
        writer.startTAG(XML_TAG);
        super.writeXML(writer);
        try {
            writer.attr("field1", field);
            writer.attr("unit", unit);
        } catch (Exception e) {
        }
        writer.end();

    }
    
	private static DateGetter getDateGetter(int unit){
		switch(unit){
			case DateConstant.DATE.YEAR :  return YearGetter.INSTANCE;
			case DateConstant.DATE.MONTH :  return MonthGetter.INSTANCE;
			case DateConstant.DATE.SEASON :  return SeasonGetter.INSTANCE;
			default : {
				return getDateGetter(DateConstant.DATE.YEAR);
			}
		}
	}
	
	@Override
	protected int getClassType(){
		return DBConstant.CLASS.INTEGER;
	}
}