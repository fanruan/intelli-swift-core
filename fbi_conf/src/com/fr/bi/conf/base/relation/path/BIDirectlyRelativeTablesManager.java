 ...
 */
abstract class BIDirectlyRelativeTablesManager extends BIHashMapContainer<Table, BIDirectlyRelativeTableContainer> {
    ...
    protected abstract BIDirectlyRelativeTableContainer generateDirectTableContainer(Table table);

    private BIDirectlyRelativeTableContainer getDirectTableContainer(Table table) {
        try {
    ...
    BITableContainer getIndirectTable(Table table) {
    ...
    protected void updateIndirectContainer() {
        Iterator<Table> it = keySet().iterator();
        while (it.hasNext()) {
            Table table = it.next();
