 ...
 * 字段类型date
 */
BI.AbstractDimensionCombo = BI.inherit(BI.Widget, {
    ...
    _init: function(){
        ...
        this.combo.on(BI.DownListCombo.EVENT_CHANGE, function(v){
            self.fireEvent(BI.AbstractDimensionCombo.EVENT_CHANGE, v);
        });

        this.combo.on(BI.DownListCombo.EVENT_SON_VALUE_CHANGE,function(v, father){
            self.fireEvent(BI.AbstractDimensionCombo.EVENT_CHANGE, father, v);
        });

        this.combo.on(BI.DownListCombo.EVENT_BEFORE_POPUPVIEW,function(){
            this.populate(self._rebuildItems());
            var selectedValue = self._createValueBySortAndGroup(BI.Utils.getDimensionSortByID(o.dId),BI.Utils.getDimensionGroupByID(o.dId));
            this.setValue([selectedValue.sort,selectedValue.group]);
        });
    },
    ...
    _rebuildItems:function(){
        ...
        var selectedValues = this._createValueBySortAndGroup(BI.Utils.getDimensionSortByID(o.dId),BI.Utils.getDimensionGroupByID(o.dId));

        switch (selectedValues.sort.value) {         //先更新老子的文本
            case this.typeConfig().ASCEND :
                this._changeElText(ascend.el,BI.has(selectedValues.sort, "childValue") ? BI.i18nText("BI-Ascend") + "(" + BI.Utils.getDimensionNameByID(selectedValues.sort.childValue) + ")" : BI.i18nText("BI-Ascend"));
            ...
            case this.typeConfig().DESCEND :
                ...
                this._changeElText(descend.el,BI.has(selectedValues.sort, "childValue") ? BI.i18nText("BI-Descend") + "(" + BI.Utils.getDimensionNameByID(selectedValues.sort.childValue) + ")" : BI.i18nText("BI-Descend"));
                break;
            default :
                this._changeElText(ascend.el,BI.i18nText("BI-Ascend"));
                this._changeElText(descend.el,BI.i18nText("BI-Descend"));
    ...
    _createValueBySortAndGroup: function(sort,group){
...
BI.AbstractDimensionCombo.EVENT_CHANGE = "EVENT_CHANGE";