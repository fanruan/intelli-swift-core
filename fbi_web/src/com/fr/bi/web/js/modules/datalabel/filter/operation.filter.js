 ...
 * @extend BI.Widget
 */
BI.DataLabelFilterOperation = BI.inherit(BI.Widget, {
    _defaultConfig: function() {
        return BI.extend(BI.DataLabelFilterOperation.superclass._defaultConfig.apply(this, arguments), {
    ...
    _defaultState: function() {
        ...
        this.buttonComboTab.setSelect(BI.DataLabelFilterOperation.OPERATION_ADD_CONDITION);
    },

    _init: function() {
        BI.DataLabelFilterOperation.superclass._init.apply(this, arguments);
        var self = this,
            o = this.options;
        this.currentSelected = null;
        this.filter = BI.createWidget({
            type: "bi.data_label_filter_pane",
            expander: o.expander,
            items: o.items,
        ...
        this.filter.on(BI.Controller.EVENT_CHANGE, function(type, value, obj) {
            if (type === BI.Events.CLICK) {
                ...
                if (self.currentSelected.isSelectedCondition()) {
                    self.buttonComboTab.setSelect(BI.DataLabelFilterOperation.OPERATION_ADD_ANDOR_CONDITION);
                } else {
                    self.buttonComboTab.setSelect(BI.DataLabelFilterOperation.OPERATION_ADD_CONDITION);
                }
            }
            if (type === BI.Events.DESTROY) {
                ...
                self.fireEvent(BI.DataLabelFilterOperation.EVENT_DESTROY_ITEM, value, obj);
            }
        });
        this.filter.on(BI.DataLabelFilterPane.EVENT_CHANGE, function() {
            self.fireEvent(BI.DataLabelFilterOperation.EVENT_CHANGE, arguments);
        });
    ...
    _createTabs: function(v) {
        var self = this;
        switch (v) {
            case BI.DataLabelFilterOperation.OPERATION_ADD_CONDITION:
                ...
                btnGroup.on(BI.ButtonGroup.EVENT_CHANGE, function(value, obj) {
                    self.fireEvent(BI.DataLabelFilterOperation.EVENT_OPERATION, obj.getValue());
                    self._defaultState();
                });
                return btnGroup;
            case BI.DataLabelFilterOperation.OPERATION_ADD_ANDOR_CONDITION:
                var btnGroup = BI.createWidget({
                    type: "bi.button_group",
                    chooseType: BI.ButtonGroup.CHOOSE_TYPE_DEFAULT,
...
BI.extend(BI.DataLabelFilterOperation, {
    OPERATION_ADD_CONDITION: 0,
    OPERATION_ADD_ANDOR_CONDITION: 1
});
BI.DataLabelFilterOperation.EVENT_OPERATION = "EVENT_OPERATION";
BI.DataLabelFilterOperation.EVENT_CHANGE = "EVENT_CHANGE";
BI.DataLabelFilterOperation.EVENT_DESTROY_ITEM = "BI.DataLabelFilterOperation.EVENT_DESTROY_ITEM";
$.shortcut("bi.data_label_filter_operation", BI.DataLabelFilterOperation);
