...
public class CubeExecutor extends SheetExecutor {
    ...
    public CubeExecutor(TemplateReport report, TableWidget widget,
                        String sessionID,
    ...
    @Override
    public ResultBlock execute4Poly(Actor actor, long startTime) {
        BIPolyAnalyECBlock block = null;
        block = new BIPolyAnalyECBlock();
        try {
        ...
        FRContext.getLogger().error(DateUtils.timeCostFrom(startTime) + " beb time");
        release();
        return block;
    }
    ...
    protected ResultECReport execute4ECReport(int execute_type, long startTime) {
        ...
        FRContext.getLogger().error(DateUtils.timeCostFrom(s) + " p2");
