package com.fr.bi.cal.analyze.report.report.widget.chart.style.filter.number;
 ...
 */
public class NumberEqualFilter implements IFilter{

    private double value;

    public NumberEqualFilter(double value) {
        this.value = value;
    }

    private boolean isNumberEqual(double v){
        if (Double.isNaN(v)) {
            return false;
        }
    ...
    public JSONArray getFilterResult(JSONArray array) throws JSONException {
        ...
        for(int i = 0; i < array.length(); i++){
            if(isNumberEqual(array.getDouble(i))){
