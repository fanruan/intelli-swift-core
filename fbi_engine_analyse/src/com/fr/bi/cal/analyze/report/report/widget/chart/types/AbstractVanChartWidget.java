...
/**
 * init(BIWidgetConf widgetConf, JSONObject data);
 * calculateSCData();
 * Created by AstronautOO7 on 2017/7/15.
 */
public abstract class AbstractVanChartWidget implements SclCalculator {
    ...
    //考虑各种自动、透明之后真实的图表背景
    private String realBackgroundColor;
...
//        super.parseJSON(jo, userId);
    ...
    public JSONObject createPlotOptions(JSONObject globalStyle, JSONObject settings) throws Exception {
        ...
        plotOptions.put("tooltip", createTooltip(settings));
    ...
    protected String numberFormat(String dimensionID, double y) {
        y = checkInfinity(y);
        try {
            //tableWidget里的getBITargetByID为了得到指标的样式设置,所以这里通过指标的id得到指标样式
            int type = widgetConf.getFormatDecimalByTargetID(dimensionID);//默认为自动
            String format;
            switch (type) {
    ...
    protected int numberLevelFromSettings(String dimensionID) {
        try {
            return widgetConf.getNumberLevelByTargetID(dimensionID);
        } catch (Exception e) {
            BILoggerFactory.getLogger().error(e.getMessage(), e);
        }
    ...
    private String getRealBackgroundColor() {
    ...
    private String backgroundColor() {
        ...
        JSONObject settings = widgetConf.getWidgetSettings().getDetailSettings();
    ...
    public JSONObject calculateSCData(BIWidgetConf widgetConf, JSONObject data) throws Exception {
        ...
        JSONObject jo = this.createOptions(globalStyle, data).put("data", data);

        // 如果是大数据模式,而且分组数大于BigDataChartOperator.MAXROW 或者是图表是交叉表类型且top分组大于BigDataChartOperator.MAXROW
        boolean isLarge = (data.has("c") && data.getJSONArray("c").length() > MAXROW)
                || (data.has("t") && data.getJSONObject("t").has("c") && data.getJSONObject("t").getJSONArray("c").length() > MAXROW);
        if (BIWidgetConfUtils.needOpenBigDateModel(widgetConf) && isLarge) {
            jo.put("chartBigDataModel", true);
        }
    ...
    * */
    ...
    protected String tooltipValueFormat(String dimensionID) {
        return this.valueFormatFunc(dimensionID, true);
    }
    ...
    protected String decimalFormat(String dimensionID, boolean hasSeparator) {
        int type = widgetConf.getFormatDecimalByTargetID(dimensionID);//默认为自动
        String format;
        switch (type) {
            case BIReportConstant.TARGET_STYLE.FORMAT.NORMAL:
    ...
    protected String valueUnit(String dimensionID, boolean isTooltip) {
        String scaleUnit = this.scaleUnit(this.numberLevel(dimensionID));
        String unit = widgetConf.getUnitByTargetID(dimensionID);
    ...
    //小数位数和千分符，即会改变数值的
    protected String valueFormat(String dimensionID) {
        boolean hasSeparator = widgetConf.getSeparatorByTargetID(dimensionID);
        return this.decimalFormat(dimensionID, hasSeparator);
    }
    ...
    protected String getSerBITarget(JSONObject ser) throws Exception {
        JSONArray ids = ser.optJSONArray("targetIDs");
        return ids == null ? null : ids.optString(0);
    }
    ...
    private JSONArray createSeriesWithTop(JSONObject originData) throws Exception {
        String categoryID = this.getCategoryDimensionID(), seriesDimID = this.getSeriesDimensionID();
        ...
        for (int i = 0; i < topC.length(); i++) {
            ...
            if(leftC != null) {
                for (int j = 0; j < leftC.length(); j++) {
                    ...
                    String formattedCategory = this.formatDimension(categoryID, x);
    ...
    protected JSONArray createSeriesWithChildren(JSONObject originData) throws Exception {
        String category = this.getCategoryDimensionID(), seriesDim = this.getSeriesDimensionID();
    ...
    //系列指标不为空的时候创建多个系列
    protected JSONArray createSeriesWithSeriesDimension(JSONObject originData, String seriesDimID) throws Exception {
        JSONArray series = JSONArray.create();
        String[] targetIDs = this.getUsedTargetID();
        String[] dimensionIDs = this.getUsedDimensionID();
        String categoryKey = this.categoryKey(), valueKey = this.valueKey();
        ...
        for (int i = 0, len = targetIDs.length; i < len; i++) {
            ...
            if (children != null) {
                for (int j = 0, count = children.length(); j < count; j++) {
                    ...
                    JSONObject ser = JSONObject.create().put("data", JSONArray.create().put(datum)).put("name", widgetConf.getDimensionNameByDimensionID(id))
                            .put("type", type).put("yAxis", yAxis)
                            .put("dimensionIDs", dimensionIDs)
                            .put("targetIDs", JSONArray.create().put(id))
    ...
    protected String formatDimension(String dimensionID, String dimensionName) {
        if (dimensionID == null || StringUtils.isBlank(dimensionName)) {
            return dimensionName;
        }

        int groupType = widgetConf.getDimensionGroupByDimensionID(dimensionID).optInt("type", BIReportConstant.GROUP.AUTO_GROUP);
        JSONObject dateFormat = widgetConf.getDateFormatByDimensionID(dimensionID);
    ...
    protected JSONObject revertMapLegend(JSONObject settings, JSONObject legend) throws JSONException {
        ...
        String[] targetIDs = this.getTargets();
        if (targetIDs.length > 0) {
            legend.put("formatter", this.intervalLegendFormatter(this.valueFormat(targetIDs[0]), this.valueUnit(targetIDs[0], true)));
        }

        if (settings.optInt("legend") == BIChartSettingConstant.CHART_LEGENDS.BOTTOM) {
            legend.put("position", "right-bottom");
        }
    ...
    public String getCategoryDimensionID(int index) {
        List<String> dimensionIds = widgetConf.getViewMap().get(Integer.parseInt(BIReportConstant.REGION.DIMENSION1));
        if (dimensionIds == null) {
            return null;
        }

        int dIndex = 0;
        JSONObject dimensions = widgetConf.getDimensions();
        Iterator it = dimensions.keys();
        while (it.hasNext()) {
            String dId = it.next().toString();
            if (dimensionIds.contains(dId) && dimensions.optJSONObject(dId).optBoolean("used")) {
                if (dIndex == index) {
                    return dId;
                }
                dIndex++;
    ...
    public String getCategoryDimensionID() {
        return this.getCategoryDimensionID(0);
    }

    public String getSeriesDimensionID() {
        List<String> dimensionIds = widgetConf.getViewMap().get(Integer.parseInt(BIReportConstant.REGION.DIMENSION2));
        if (dimensionIds == null) {
            return null;
        }
        JSONObject dimensions = widgetConf.getDimensions();
    ...
    protected String[] getUsedTargetID() {
        Map<Integer, List<String>> view = widgetConf.getViewMap();
    ...
    protected String[] getUsedDimensionID() {
        ...
        for (Integer key : view.keySet()) {
            if (key >= Integer.parseInt(BIReportConstant.REGION.DIMENSION1) && key < Integer.parseInt(BIReportConstant.REGION.TARGET1)) {
                List<String> region = view.get(key);
                for (String id : region) {
                    if (getDimensions().get(id).optBoolean("used")) {
                        dimensionIDs.add(id);
        ...
        return dimensionIDs.toArray(new String[dimensionIDs.size()]);
    }
    ...
    protected String[] getTargets() {
        ...
        for (Integer key : view.keySet()) {
            if (key > Integer.parseInt(BIReportConstant.REGION.TARGET1)) {
                List<String> region = view.get(key);
                targetIDs.addAll(region);
            }
        }
        return targetIDs.toArray(new String[targetIDs.size()]);
    }

    protected Map<String, JSONObject> getDimensions() {
        JSONObject dimensions = widgetConf.getDimensions();
...
//    }
    ...
    //<editor-fold>=========================about plotOptions.label tooltip==========================================================
    ...
    private JSONObject createTooltip(JSONObject settings) throws JSONException {
        //tooltip的默认配置
        JSONObject tooltip = JSONObject.create();

        String widgetBg = getRealBackgroundColor();

        boolean enabled = !settings.optBoolean("bigDataMode", false);

        tooltip.put("enabled", enabled)
                .put("animation", true).put("padding", 10)
                .put("backgroundColor", imageBack(widgetBg) ? DARK : widgetBg)
                .put("borderRadius", 2).put("borderWidth", 0).put("shadow", true);

        if(enabled) {
            JSONObject tooltipSetting = this.defaultTooltipSetting();
            tooltipSetting = settings.has("tooltipStyle") ? merge(settings.optJSONObject("tooltipStyle"), tooltipSetting) : tooltipSetting;


            JSONObject formatter = JSONObject.create();
            formatter.put("identifier", labelIdentifier(tooltipSetting));

            tooltip.put("formatter", formatter);
            tooltip.put("style", tooltipSetting.optJSONObject("textStyle"));
            tooltip.put("shared", tooltipSetting.optBoolean("shared"));
        }
    ...
    //提示的默认值
    protected JSONObject defaultTooltipSetting() throws JSONException {
    ...
    private String dataLabelAlign(int position) {
    ...
    //<editor-fold>=========================about chart condition, like tooltip dataLabel image dataSheet==========================================================

    private void dealConditions(JSONObject options) throws JSONException {
        ...
        for (int i = 0, count = series.length(); i < count; i++) {
            ...
            if (targetIDs == null || ser.optBoolean(TRANS_SERIES)) {
                continue;
            }
            String targetID = targetIDs.optString(0);

            dealSerConditions(options, targetID, ser);
        }
    }

    protected void dealSerConditions(JSONObject options, String targetID, JSONObject ser) throws JSONException {
        String label = getSerLabelString(options);
        if(label != null) {
            dealSerLabelCondition(targetID, ser, label);
        }
        String tooltip = getSerTooltipString(options);
        if(tooltip != null) {
            dealSerTooltipCondition(targetID, ser, tooltip);
        }
    }

    private String getSerLabelString(JSONObject options) {
        JSONObject labelJSON = options.optJSONObject("plotOptions").optJSONObject(dataLabelsKey());

        if (labelJSON.optBoolean("enabled")) {
    ...
    protected void dealSerLabelCondition(String targetID, JSONObject ser, String serLabelString) throws JSONException {
        dealCommonSerLabelCondition(targetID, ser, serLabelString);
    }

    protected void dealCommonSerLabelCondition(String targetID, JSONObject ser, String serLabelString) throws JSONException {
        JSONObject labels = new JSONObject(serLabelString);
        labels.optJSONObject("formatter")
                .put("valueFormat", this.dataLabelValueFormat(targetID))
    ...
    private String getSerTooltipString(JSONObject options) {
        JSONObject tooltipJSON = options.optJSONObject("plotOptions").optJSONObject("tooltip");

        if (tooltipJSON.optBoolean("enabled")) {
            return tooltipJSON.toString();
        }

        return null;
    }

    protected void dealSerTooltipCondition(String targetID, JSONObject ser, String serTooltipString) throws JSONException {
        dealCommonSerTooltipCondition(targetID, ser, serTooltipString);
    }

    protected void dealCommonSerTooltipCondition(String targetID, JSONObject ser, String serTooltipString) throws JSONException {
        JSONObject tooltip = new JSONObject(serTooltipString);
        tooltip.optJSONObject("formatter")
                .put("valueFormat", this.tooltipValueFormat(targetID))
                .put("percentFormat", "function(){return BI.contentFormat(arguments[0], \"#.##%\")}")
                .put("arrivalRateFormat", "function(){return BI.contentFormat(arguments[0], \"#.##%\")}");

        ser.put("tooltip", tooltip);
    }

    //</editor-fold>=========END================about chart condition, like dataLabel image dataSheet==========================================================
