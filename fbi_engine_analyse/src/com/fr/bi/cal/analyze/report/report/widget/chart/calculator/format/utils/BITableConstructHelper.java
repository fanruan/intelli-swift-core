...
//            boolean isInnerSum = item.getChildren() != null && !item.getChildren().isEmpty();
            ...
            formatItemCell(ops, rowIndex, style, item, isLastSum);
            if (item.getValues() != null) {
                for (ITableItem it : item.getValues()) {
                    formatItemCell(ops, rowIndex, style, it, isLastSum);
                }
            }
    ...
    private static void formatItemCell(Map<String, ITableCellFormatOperation> ops, int rowIndex, BIWidgetSettings style, ITableItem item, boolean isLastSum) throws Exception {
        formatText(ops, item);
