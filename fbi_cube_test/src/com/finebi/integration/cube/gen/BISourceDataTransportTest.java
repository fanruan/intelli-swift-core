...
* 再加入:{id:3,name:Japan,id:4,name:Canada，id：5，name：Mexico}*/
    ...
    public void testNeverTransportTable() throws BICubeColumnAbsentException, URISyntaxException {
        ...
        dataTransport = new BISourceDataNeverTransportTestTool(cube, tableSource, new HashSet<CubeTableSource>(), new HashSet<CubeTableSource>());
        dataTransport.mainTask(null);
...
//            ICubeResourceRetrievalService resourceRetrievalService = new BICubeResourceRetrieval(BICubeConfiguration.getTempConf(String.valueOf(UserControl.getInstance().getSuperManagerID())));
            ...
            dataTransport = new BISourceDataPartTransportTestTool(cube, tableSource, new HashSet<CubeTableSource>(), new HashSet<CubeTableSource>(), oldCount);
            dataTransport.mainTask(null);
        } catch (Exception e) {
            e.printStackTrace();
            assertTrue(false);
