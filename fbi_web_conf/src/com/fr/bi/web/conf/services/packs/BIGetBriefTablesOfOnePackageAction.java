        ...
        if (pack == null) {
            ...
            for (int i = 0; i < tables.length(); i++) {
                ...
                for (int j = 0; j < fieldList.size(); j++) {
                    JSONObject field = fieldList.get(j).createJSON();
                    field.put(BIJSONConstant.JSON_KEYS.TRAN_NAME, BICubeConfigureCenter.getAliasManager().getAliasName(field.getString("id"), userId));
                ...
                sourceTable.put(BIJSONConstant.JSON_KEYS.TRAN_NAME, BICubeConfigureCenter.getAliasManager().getAliasName(tableId, userId));
