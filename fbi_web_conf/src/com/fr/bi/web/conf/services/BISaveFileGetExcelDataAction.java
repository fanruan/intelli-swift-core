...
public class BISaveFileGetExcelDataAction extends AbstractBIConfigureAction {
    ...
    protected void actionCMDPrivilegePassed(HttpServletRequest req, HttpServletResponse res) throws Exception {
        ...
        if (attach != null) {
            ...
            boolean isExcel = reportName.endsWith(".xls") || reportName.endsWith(".xlsx") || reportName.endsWith(".csv");
            if (isExcel) {
                ...
                File file = new File(parentFile, fileId + reportName);
                FileOutputStream fs = new FileOutputStream(file);
                fs.write(bytes);
                fs.flush();
                fs.close();
                BIGetImportedExcelData excelTableData = new BIGetImportedExcelData(file.getName());
                WebUtils.printAsJSON(res, excelTableData.getFieldsAndPreviewData());
            }
        }
    }
