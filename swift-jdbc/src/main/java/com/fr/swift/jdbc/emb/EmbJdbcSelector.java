            ...
            while (!stop.get()) {
                while (connector.isNeedToSend()) {
                    ...
                    try {
                        ...
                        if (null == proxyObject) {
                            response.setException(new SwiftJDBCNotSupportedException(request.getInterfaceName() + " is invalid service"));
                        } else {
                            ...
                            if (null != api && api.enable()) {
                                ...
                                response.setException(new SwiftJDBCNotSupportedException(method.getName() + " is invalid service"));
                            }
                        }
                    } catch (Exception e) {
                        response.setException(new SwiftJDBCNotSupportedException("Service invalid. ", e));
                    }
