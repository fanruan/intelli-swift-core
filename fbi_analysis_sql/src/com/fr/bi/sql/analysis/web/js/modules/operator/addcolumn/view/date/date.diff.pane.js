...
BI.AnalysisETLOperatorAddColumnDateDiffPane = BI.inherit(BI.MVCWidget, {
    ...
    _initView : function () {
        ...
        self.segment = BI.createWidget({
            type : 'bi.segment',
            items : SQLCst.ANALYSIS_ADD_COLUMN_DATE_DIFF_TYPE_ITEMS
        });
        self.segment.on(BI.Segment.EVENT_CHANGE, function (type) {
            self.controller.setType(self.segment.getValue()[0]);
        });
...
$.shortcut(SQLCst.ANALYSIS_ETL_PAGES.ADD_COLUMN + '_' + BICst.ETL_ADD_COLUMN_TYPE.DATE_DIFF, BI.AnalysisETLOperatorAddColumnDateDiffPane);