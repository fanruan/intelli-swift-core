...
BI.AnalysisETLOperatorAddColumnDateDiffController = BI.inherit(BI.MVCController, {
    ...
    populate : function (widget, model) {
        ...
        fields.push({
            ...
            value:SQLCst.SYSTEM_TIME
        })
        BI.each(model.get(SQLCst.FIELDS) || [], function (idx, item) {
        ...
        model.set(SQLCst.FIELDS, fields)
        widget.segment.setValue(model.get('type') || SQLCst.ANALYSIS_ADD_COLUMN_DATE_DIFF_TYPE.YEAR);
        model.set("type", widget.segment.getValue()[0])
        widget.lcombo.populate(model.get(SQLCst.FIELDS));
        widget.rcombo.populate(model.get(SQLCst.FIELDS));
