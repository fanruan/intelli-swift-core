BI.AnalysisETLOperatorAddColumnPaneModel = BI.inherit(BI.MVCModel, {
    ...
    createFields : function () {
        if(this.valid === true) {
            ...
            return BI.concat(parent[SQLCst.FIELDS], BI.map(this.getAddColumns(), function (idx, item) {
                return {
        ...
        BI.some(columns, function(i ,column){
            switch (column.add_column_type){
                case BICst.ETL_ADD_COLUMN_TYPE.FORMULA :
                    found =  self._checkFormula(column, parent[SQLCst.FIELDS])
                    return found[0];
                case BICst.ETL_ADD_COLUMN_TYPE.DATE_DIFF :
                    var fields = [];
                    var f = column.item['firstField'];
                    if(f !== SQLCst.SYSTEM_TIME) {
                        fields.push(f);
                    }
                    ...
                    found = self._checkField(fields, parent[SQLCst.FIELDS],column.field_name, BICst.COLUMN.DATE);
                    return found[0];
                ...
                case BICst.ETL_ADD_COLUMN_TYPE.DATE_YEAR :
                    found = self._checkField([column.item['field']], parent[SQLCst.FIELDS],column.field_name,BICst.COLUMN.DATE)
                    return found[0];
                ...
                case BICst.ETL_ADD_COLUMN_TYPE.EXPR_CPP_PERCENT:
                    ...
                    found =  self._checkField(fields, parent[SQLCst.FIELDS],column.field_name,BICst.COLUMN.NUMBER);
                    return found[0];
                ...
                case BICst.ETL_ADD_COLUMN_TYPE.EXPR_SUM:
                    found =  self._checkField([column.item['field']], parent[SQLCst.FIELDS],column.field_name,BICst.COLUMN.NUMBER);
                    return found[0];
                case BICst.ETL_ADD_COLUMN_TYPE.GROUP:
                    return BI.some(column.item['items'], function (i, item) {
                        found =  self._checkField([item['field']["value"]], parent[SQLCst.FIELDS],column.field_name, item['field']['fieldType'])
                        if(found[0] === true) {
                            return true;
                        }
                    })
    ...
    update : function () {
        ...
        v.etlType = SQLCst.ETL_TYPE.ADD_COLUMN;
        v[SQLCst.FIELDS] = this.createFields();
        v.operator = {};
...
SQLCst.OPERATOR_MODEL_CLASS[SQLCst.ANALYSIS_ETL_PAGES.ADD_COLUMN] = BI.AnalysisETLOperatorAddColumnPaneModel;