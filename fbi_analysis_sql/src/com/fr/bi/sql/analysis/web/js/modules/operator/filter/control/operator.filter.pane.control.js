BI.AnalysisETLOperatorFilterPaneController = BI.inherit(BI.MVCController, {
    ...
    _check : function (widget, model) {
        ...
        if(invalid[0] === true) {
            ...
            widget.fireEvent(BI.TopPointerSavePane.EVENT_FIELD_VALID, BI.deepClone(parent[SQLCst.FIELDS]))
        }
    ...
    update : function (widget, model) {
        var v =  model.update();
        v.etlType = SQLCst.ETL_TYPE.FILTER;
        var parent = model.get(SQLCst.PARENTS)[0];
        v[SQLCst.FIELDS] = BI.deepClone(parent[SQLCst.FIELDS])
        return v;
    }
