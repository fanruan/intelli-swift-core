BI.AnalysisETLOperatorFilterPaneModel = BI.inherit(BI.MVCModel, {
    ...
    check : function () {
        ...
        var items =operator[SQLCst.ITEMS];
        var newItems = [];
        var invalid = [false];
        BI.each(items, function (i, item) {
            var field = BI.find(parent[SQLCst.FIELDS], function (i, field) {
            ...
            if (BI.isNotNull(field)){
                ...
                if (!invalid[0]){
                    if (field.field_type !== item.field_type){
                        ...
                        invalid = self._checkItem(item, parent[SQLCst.FIELDS]);
                    }
                }
            }
        });
...
SQLCst.OPERATOR_MODEL_CLASS[SQLCst.ANALYSIS_ETL_PAGES.FILTER] =  BI.AnalysisETLOperatorFilterPaneModel