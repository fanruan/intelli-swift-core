BI.DynamictabController = BI.inherit(BI.MVCController, {
    ...
    dealWithCombo : function (v, button, widget, model) {
        var self = this;
        switch (v) {
            case SQLCst.ANALYSIS_TABLE_SET.DELETE:{
                var items = model.get(SQLCst.ITEMS);
                if(items.length === 1) {
            ...
            case SQLCst.ANALYSIS_TABLE_SET.COPY:{
                this._copySheet(button.getValue(), widget, model);
                break;
            }
            case SQLCst.ANALYSIS_TABLE_SET.RENAME:{
                var id = button.getValue();
    ...
    _refreshButtonStatus : function (widget, model) {
        this._getButtonController(widget).setMergeEnable(model.getValue(SQLCst.ITEMS).length > 1);
    },
    ...
    _createTables : function (widget, model) {
        return model.update()[SQLCst.ITEMS];
    },

    deleteMergeSheet : function (v, widget, model) {
        var oldMergeTable = model.getSheetData(v);
        var parents = oldMergeTable[SQLCst.PARENTS];
        while(parents.length === 1) {
            parents = parents[0][SQLCst.PARENTS];
        }
        var self = this;
    ...
    chooseSheetForMerge : function (widget, model) {
        var tabs = model.get(SQLCst.ITEMS)
        ...
        var func = function (v) {
            ...
            m[SQLCst.PARENTS] = getTablesBySheetId(v);
            BI.createWidget({
        ...
        if(items.length === 2) {
            ...
            popover.on(BI.PopoverSection.EVENT_CLOSE, function () {
                BI.Layers.hide(SQLCst.ANALYSIS_POPUP_FOLATBOX_LAYER);
            })
            popover.on(BI.AnalysisETLMergeSheetPopover.EVENT_CHANGE, func);
            BI.Popovers.remove("etlChooseSheetForMerge");
            BI.Popovers.create("etlChooseSheetForMerge", popover, {width : 500, height : 400, container: BI.Layers.create(SQLCst.ANALYSIS_POPUP_FOLATBOX_LAYER)}).open("etlChooseSheetForMerge");
            BI.Layers.show(SQLCst.ANALYSIS_POPUP_FOLATBOX_LAYER)
        }
    },
    ...
    populate : function (widget, model) {
        var items = model.getValue(SQLCst.ITEMS);
