BI.AnalysisETLMergeSheetFieldsModel = BI.inherit(BI.MVCModel, {
    ...
    _init : function () {
        ...
        var fields = this.get(SQLCst.FIELDS);
        if(BI.isNull(fields)){
            this.reset();
        }
    },
    ...
    reset : function () {
        ...
        this.set(SQLCst.FIELDS, fields);
    },

    isValid : function () {
        var joinFields = this.getValue(SQLCst.FIELDS);
        ...
        var table1 = tables[0];
        var table2 = tables[1];
        var table1Fields = BI.Utils.getSQLFieldArrayFromTable(table1);
        var table2Fields = BI.Utils.getSQLFieldArrayFromTable(table2);
    ...
    getAllTables : function () {
        ...
        return BI.map(tables, function (idx, item) {
            return BI.extend(item, {
                fields:[item[SQLCst.FIELDS] || [],[],[]]
            })
        })
    },

    getAllFields : function () {
        var mergeFields = this.getValue(SQLCst.FIELDS);
        var tables = this.get("tables")
        var table1 = tables[0];
        var table2 = tables[1];
        var table1Fields = BI.Utils.getSQLFieldArrayFromTable(table1);
