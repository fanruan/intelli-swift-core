BI.HistoryTabModel = BI.inherit(BI.MVCModel, {
    ...
    _init : function () {
        ...
        if (BI.isNull(this.options.etlType)){
            ...
            this.addItemAfter(SQLCst.ANALYSIS_TABLE_HISTORY_TABLE_MAP.CHOOSE_FIELD, -1, new BI.AnalysisETLOperatorSelectDataModel().update())
        } else {
    ...
    _initItems : function (table, items) {
        var operator = SQLCst.ANALYSIS_TABLE_OPERATOR_KEY[table.etlType];
        var self = this;
        items = BI.concat([{
            op : operator,
            table : table
    ...
    _initId : function (tables) {
        ...
        if(BI.isNotNull(tables)) {
            BI.each(tables, function (idx, item) {
                ...
                var operator = SQLCst.ANALYSIS_TABLE_OPERATOR_KEY[item.etlType]
                BI.extend(item, {
                    operatorType : operator["operatorType"],
                    text:operator["text"]
                })
                if(item.etlType === SQLCst.ETL_TYPE.SELECT_DATA){
                    item["operator"] = new BI.AnalysisETLOperatorSelectDataModel(item).update()["operator"]
                }
                self._initId(item[SQLCst.PARENTS])
            })
        }
    },
    ...
    addItemAfter : function (v, index, options) {
        ...
        var items = this.get(SQLCst.ITEMS);
        if (items.length !== 0){
    ...
    checkBeforeSave : function (table, index) {
        ...
        for(var i = index; i < items.length; i++){
            ...
            c.parents = [p];
            var modelClass = SQLCst.OPERATOR_MODEL_CLASS[c.operatorType];
            var m = new modelClass(c);
            var res = m.check();
            if(res[0] === true){
                return [false, BI.i18nText("BI-Step") + ":" + (i + 1) + SQLCst.ANALYSIS_TABLE_OPERATOR_KEY[c.etlType].text  + " "+ res[1] + "!!!" +
    ...
    isModelValid : function () {
        return this.getValue('invalidIndex') >= this.get(SQLCst.ITEMS).length
    },
    ...
    _toArray : function (model) {
        ...
        items.push(item);
        var parents = model[SQLCst.PARENTS];
        var self = this;
