BI.AnalysisETLMergeSheetModel = BI.inherit(BI.MVCModel, {
    ...
    _init : function () {
        ...
        var baseTable = this.get("tables") || this.get(SQLCst.PARENTS);
        this.set("tables", baseTable)
        this.set("sheets", this._getSheets())
        var tables = this.get(SQLCst.PARENTS)
        var m = {tables:tables};
        m[SQLCst.FIELDS] = this._createMergeFields();
    ...
    getMergeFieldsName : function () {
        var fields = this.get(BI.AnalysisETLMergeSheetModel.MERGE_FIELDS).getValue(SQLCst.FIELDS);
    ...
    _createNameArray : function (fields) {
        ...
        var allFields = BI.map(tables, function (idx, item) {
            return BI.Utils.getSQLFieldArrayFromTable(item);
        })
    ...
    _getSheets : function () {
        return BI.map(this.get(SQLCst.PARENTS), function (idx, item) {
            return item.value;
        })
    },
    ...
    setCurrent2Sheet : function (v) {
        ...
        if(changed === true) {
            ...
            this.set(SQLCst.PARENTS, tables);
    ...
    _getFieldIndex : function (idx, field_name) {
        return BI.findIndex(this.get(SQLCst.PARENTS)[idx][SQLCst.FIELDS], function (idx, item) {
    ...
    refreshColumnName : function () {
        ...
        var columns = [];
        var tables = this.get(SQLCst.PARENTS);
        var table1Fields = BI.Utils.getSQLFieldArrayFromTable(tables[0]);
        var table2Fields = BI.Utils.getSQLFieldArrayFromTable(tables[1]);
        ...
        BI.each(allFields, function (idx, item) {
            ...
            newFields.field_name = BI.Utils.createSQLDistinctName(getNameArray(columns), BI.isNotNull(oldColumn) ? oldColumn.field_name : newFields.field_name)
    ...
    update : function () {
        ...
        json[SQLCst.FIELDS] = json["columns"]
        json["etlType"] = SQLCst.ETL_TYPE.MERGE_SHEET;
        delete json["tables"];
