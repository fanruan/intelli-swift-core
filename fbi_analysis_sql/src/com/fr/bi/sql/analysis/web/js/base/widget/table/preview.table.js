...
BI.AnalysisETLPreviewTable = BI.inherit(BI.Widget, {
    ...
    _defaultConfig: function() {
        ...
        return BI.extend(conf, {
            ...
            operator:SQLCst.ANALYSIS_ETL_PAGES.SELECT_DATA
        })
    },
    ...
    _initDrag : function() {
        ...
        self.element.droppable({
            accept:"."+ SQLCst.ANALYSIS_DRAG_CLASS,
            drop : function(e, ui) {
                var absolutePosition = ui.position.left + self.table.getRightHorizontalScroll() + (e.pageX - ui.helper.offset().left);
        ...
        this.table.element.hover(function(e){
            if(self.options.operator === SQLCst.ANALYSIS_ETL_PAGES.SELECT_DATA){
    ...
    _isDragButton : function($el) {
        return $el.hasClass(SQLCst.ANALYSIS_DRAG_CLASS)
            || $el.parents("."+SQLCst.ANALYSIS_DRAG_CLASS, this.table.element).length > 0
    },
    ...
    _createHeader : function (header) {
        ...
        switch (operator) {
            case  SQLCst.ANALYSIS_ETL_PAGES.SELECT_DATA : {
                widgetType = BI.ANALYSIS_ETL_HEADER.DELETE;
                break;
            }
            case  SQLCst.ANALYSIS_ETL_PAGES.FILTER : {
        ...
        return  BI.map([header], function (i, items) {
            return BI.map(items, function (j, item) {
                return BI.extend({
                    type: SQLCst.ANALYSIS_TABLE_OPERATOR_PREVIEW_HEADER + widgetType,
    ...
    _getNullText: function () {
        switch (this.options.operator) {
            case SQLCst.ANALYSIS_ETL_PAGES.USE_PART_FIELDS:
    ...
    _showCard : function () {
        if(this.options.operator === SQLCst.ANALYSIS_TABLE_OPERATOR_KEY.ERROR) {
