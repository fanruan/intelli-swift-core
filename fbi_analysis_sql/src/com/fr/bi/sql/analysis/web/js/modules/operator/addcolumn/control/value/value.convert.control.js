...
BI.AnalysisETLOperatorAddColumnValueConvertController = BI.inherit(BI.MVCController, {
    ...
    setField : function (field, widget, model) {
        ...
        widget.segment.setEnabledValue(SQLCst.ANALYSIS_ADD_COLUMN_CONVERT_ACCEPT_FIELDS[f.fieldType])
        var type = SQLCst.ANALYSIS_ADD_COLUMN_CONVERT_ACCEPT_FIELDS[f.fieldType][0];
    ...
    populate : function (widget, model) {
        ...
        if(BI.isNotNull(field)) {
            ...
            model.set('field', field);
            widget.segment.setEnabledValue(SQLCst.ANALYSIS_ADD_COLUMN_CONVERT_ACCEPT_FIELDS[f.fieldType]);
            var type = model.get('field_type') || SQLCst.ANALYSIS_ADD_COLUMN_CONVERT_ACCEPT_FIELDS[f.fieldType][0];
            widget.segment.setValue(type);
            model.set('field_type', type);
        }
        this._checkCanSave(widget, model);
