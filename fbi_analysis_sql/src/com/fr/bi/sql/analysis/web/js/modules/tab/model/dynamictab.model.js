BI.DynamictabModel = BI.inherit(BI.MVCModel, {
    ...
    _init : function () {
        ...
        var self = this, items = this.get(SQLCst.ITEMS);
    ...
    _getChildParents : function (id) {
        ...
        return BI.isNotNull(childModel) ? (childModel.get(SQLCst.PARENTS) || [] ): []
    },
    ...
    update : function () {
        ...
        BI.each(this.get(SQLCst.ITEMS), function (i, item) {
            tables.push(self.get(item).update())
        })
        var res = {};
        res[SQLCst.ITEMS]= tables;
        return res;
    }
