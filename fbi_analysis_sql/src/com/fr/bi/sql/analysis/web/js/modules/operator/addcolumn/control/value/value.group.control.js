...
BI.AnalysisETLOperatorAddColumnValueGroupController = BI.inherit(BI.MVCController, {
    ...
    _checkCanSave : function (widget, model) {
        return BI.isNull(BI.find(this.childValid, function (idx, item) {
            return item === false;
        })) && (this.editorValid === true || !widget.checkBox.isSelected()) && (BI.isNotNull(model.get(SQLCst.ITEMS)) && model.get(SQLCst.ITEMS).length > 0);
    },
    ...
    populate : function (widget, model) {
        var fields = model.get(SQLCst.FIELDS)
    ...
    _addCondition : function (field, value, widget, model) {
        var pane =  widget.pane.addItem(widget.createItem(field, value, this.options.field_type, model.get(SQLCst.PARENTS)));
