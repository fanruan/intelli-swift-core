...
BI.SelectDataLevel8NodeController = BI.inherit(BI.Controller, {
    ...
    _renameChecker : function (v, id) {
        return !BI.Utils.getAllSQLTableNames(id).contains(v);
    },
    ...
    startChecker : function (id) {
        ...
        var checker =  setInterval(function () {
            BI.ETLReq.reqTableStatus({
                ...
                if (res[SQLCst.GENERATED_PERCENT] === 1){
                    self.widget.showLoading(res[SQLCst.GENERATED_PERCENT]);
    ...
    _showRenamePop : function (id, text) {
        ...
        BI.Popovers.create("etlTableName", namePopover, {width : 450, height : 370, container: BI.Layers.create(SQLCst.ANALYSIS_POPUP_FOLATBOX_LAYER)}).open("etlTableName");
    ...
    afterClickList : function (v, option) {
        var self = this;
        switch (v){
            case SQLCst.ANALYSIS_TABLE_SET.EDIT :
            ...
            case SQLCst.ANALYSIS_TABLE_SET.COPY :
                BI.ETLReq.reqSaveTable({id: option.id,new_id : BI.UUID(),name : BI.Utils.createSQLDistinctName(BI.Utils.getAllSQLTableNames(), option.text)}, BI.emptyFn);
                return;
        }
