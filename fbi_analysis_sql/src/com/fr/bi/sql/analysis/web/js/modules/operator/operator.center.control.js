BI.AnalysisETLOperatorCenterController = BI.inherit(BI.MVCController, {
    ...
    doSave : function(widget){
        ...
        self.refreshPreviewData(SQLCst.ANALYSIS_TABLE_OPERATOR_KEY.NULL, widget)
    ...
    doPreviewChange : function (m, type, widget, model) {
        var type = (this._editing || type === SQLCst.ANALYSIS_TABLE_OPERATOR_KEY.ERROR ) ? type : SQLCst.ANALYSIS_TABLE_OPERATOR_KEY.NULL
        this.isError = (type === SQLCst.ANALYSIS_TABLE_OPERATOR_KEY.ERROR)
    ...
    refreshPreviewData : function (v, widget) {
        if(this.isError === true){
            v = SQLCst.ANALYSIS_TABLE_OPERATOR_KEY.ERROR;
        }
    ...
    fieldValuesCreator : function (field, callback, widget, model) {
        var table = {};
        table[SQLCst.ITEMS] = this._editing === true ? model.update()[SQLCst.PARENTS] : [model.update()];
