...
public class CalTargetParseUtils {
    ...
    private static Pair<String, GroupTarget> parseTargetCalInfoFromCalTargetField(WidgetBeanField field, String id,
        ...
        if (isIdOfBaseTargetOrBaseCalTargetField(id, widget)) {
            ...
            if (aggregatorType == null) {
                ...
                GroupTarget calTarget = GroupTargetFactory.createFromCalTarget(calTargetType, 0, paramIndexes, resultIndex);
            ...
            for (int i = 0; i < paramIndexes.length; i++) {
                ...
                WidgetBeanField subField = widget.getFieldByFieldId(fieldIds.get(i));
            ...
            // TODO: 2018/5/3 公式还要加其他信息，比如表达式字符串
            GroupTarget calTarget = GroupTargetFactory.createFromCalTarget(calTargetType, 0, new int[]{paramIndex}, resultIndex);
            return Pair.of(field.getId(), calTarget);
        }
        return null;
    }
    ...
    private static Set<Pair<String, Pair<AggregatorType, FilterInfo>>> getBaseMetrics(List<String> topologicalOrder,
        ...
        for (String id : topologicalOrder) {
            ...
            FineTarget target = widget.getTargetByTargetId(id);
            if (target != null && target.getWidgetBeanField() == null) {
                // 分组表中原始字段生成的指标
                ...
                pairs.addAll(parseMetricFromBaseCalTargetField(widget.getFieldByFieldId(id), widget));
            }
        }
        return pairs;
    }
    ...
    private static Set<Pair<String, Pair<AggregatorType, FilterInfo>>> parseMetricFromBaseCalTargetField(
        ...
        for (String fieldId : field.getTargetIds()) {
            if (widget.getFieldByFieldId(fieldId).getCalculate() == null) {
                ...
                // 基础计算指标字段
                pairs.add((Pair<String, Pair<AggregatorType, FilterInfo>>) parseMetricFromBaseCalTargetField(widget.getFieldByFieldId(fieldId), widget).toArray()[0]);
            }
        }
        return pairs;
    }
