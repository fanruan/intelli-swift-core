...
public class BICubeYearColumn extends BICubeDateSubColumn<Integer> {
    ...
    protected Integer convertDate(Long date) {
        ...
        if(BICollectionUtils.isCubeNullKey(date)){
            ...
            return (Integer) ValueConverterFactory.createDateValueConverter(DateConstant.DATE.YEAR).result2Value(date);
        }
    }

    @Override
    protected Integer convertDate(Long date, Calendar calendar) {
        //return date != null ? (Integer) ValueConverterFactory.createDateValueConverter(DateConstant.DATE.YEAR).result2Value(date, calendar) : null;
        if(BICollectionUtils.isCubeNullKey(date)){
            return NIOConstant.INTEGER.NULL_VALUE;
        }else{
            return (Integer) ValueConverterFactory.createDateValueConverter(DateConstant.DATE.YEAR).result2Value(date, calendar);
        }
    }
