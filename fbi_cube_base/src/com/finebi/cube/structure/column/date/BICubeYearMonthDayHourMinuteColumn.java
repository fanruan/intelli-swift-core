...
public class BICubeYearMonthDayHourMinuteColumn extends BICubeDateSubColumn<Long> {
    ...
    protected Long convertDate(Long date) {
        ...
        if(BICollectionUtils.isCubeNullKey(date)){
            ...
            return (Long) ValueConverterFactory.createDateValueConverter(DateConstant.DATE.YEAR_MONTH_DAY_HOUR_MINUTE).result2Value(date)  ;
        }
    }
    ...
    protected Long convertDate(Long date, Calendar calendar) {
        ...
        if(BICollectionUtils.isCubeNullKey(date)){
            ...
            return (Long) ValueConverterFactory.createDateValueConverter(DateConstant.DATE.YEAR_MONTH_DAY_HOUR_MINUTE).result2Value(date, calendar) ;
        }
    }
