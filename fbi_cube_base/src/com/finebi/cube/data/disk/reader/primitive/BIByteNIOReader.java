...
public class BIByteNIOReader extends BIBasicNIOReader implements ICubeByteReader {
    ...
    public byte getSpecificValue(long filePosition) throws BIResourceInvalidException {
        try {
            ...
            throw new RuntimeException("the file is: " + baseFile + " the expect position value is: " + getIndex(filePosition) + " and the current capacity  value is: " + byteBufferArray[getPage(filePosition)].capacity() + " and the pageIndex is :" + getPage(filePosition) + e, e);
        } finally {
        }
    }
