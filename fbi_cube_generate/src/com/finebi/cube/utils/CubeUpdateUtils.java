...
public class CubeUpdateUtils {
    ...
    public static boolean isPart(long userId) {
        ...
        boolean isPart = (CubeUpdateUtils.getAllCubeAbsentTables(userId).size() > 0 || CubeUpdateUtils.getCubeAbsentRelations(userId).size() > 0 || CubeUpdateUtils.getCubeAbsentPaths(userId).size() > 0);
        return isPart;
    }
