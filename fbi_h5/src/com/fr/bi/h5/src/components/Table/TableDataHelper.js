import {isNil, each, some, UUID, Tree, remove} from 'core'
import {Widget, Dimension, Target} from 'data'
export default class TableDataHelper {
    constructor(props) {
        this.$$widget = props.$$widget;
        this.widget = new Widget(this.$$widget);
        this.EMPTY_VALUE = UUID();
        this._refreshDimsInfo();

        //展开的节点的树结构，需要保存
        this.tree = new Tree();
        this.crossTree = new Tree();

        this.page = [0, 0, 0, 0, 0];
        this.eTree = new Tree();         //展开节点——维度
        this.crossETree = new Tree();    //展开节点——系列，用于交叉表
    ...
    getColumnSize() {
        var columnSize = [];
        each(this.columnSize, function (i, size) {
            if (size < 80) {
                size = 80;
            }
    ...
    getExtraInfo() {
        ...
        op.expander = {
            x: {
                type: this.widget.getWSOpenColNode(),
                value: [this._formatExpanderTree(this.crossETree.toJSONWithNode())]
            },
            y: {
                type: this.widget.getWSOpenRowNode(),
    ...
    _refreshDimsInfo() {
        ...
        this.dimIds = [];
        this.crossDimIds = [];
        var view = this.widget.getWidgetViewByID(this.wId);
        var drill = this.widget.getDrillByID(this.wId);

        each(view[BICst.REGION.DIMENSION1], (dId)=> {
            this.widget.isDimensionUsable(dId) && (this.dimIds.push(dId));
        });
        each(view[BICst.REGION.DIMENSION2], (dId)=> {
            this.widget.isDimensionUsable(dId) && (this.crossDimIds.push(dId));
        });
        each(drill, (drArray, drId)=> {
            if (drArray.length !== 0) {
                var dIndex = this.dimIds.indexOf(drId), cIndex = this.crossDimIds.indexOf(drId);
                remove(this.dimIds, n=>n === drId);
                remove(self.crossDimIds, n=>n === drId);
                each(drArray, (dr, i)=> {
                    var tempDrId = dr.dId;
                    if (i === drArray.length - 1) {
                        if (this.widget.getRegionTypeByDimensionID(drId) === BICst.REGION.DIMENSION1) {
                            this.dimIds.splice(dIndex, 0, tempDrId);
                        } else {
                            this.crossDimIds.splice(cIndex, 0, tempDrId);
                        }
                    } else {
                        remove(this.dimIds, n=>n === tempDrId);
                        remove(this.crossDimIds, n=>n === tempDrId);
                    }
                });
            }
        });
        ...
        //使用中的指标
        this.targetIds = [];
        each(view[BICst.REGION.TARGET1], (dId)=> {
            this.widget.isDimensionUsable(dId) && (this.targetIds.push(dId));
        });
    }
    ...
    _resetPartAttrs() {
        ...
        this.showNumber = this.widget.getWSShowNumberByID();         //显示行号
        this.showRowTotal = this.widget.getWSShowRowTotalByID();    //显示行汇总
        this.showColTotal = this.widget.getWSShowColTotalByID();    //显示列汇总
        this.openRowNode = this.widget.getWSOpenRowNodeByID();      //展开所有行表头节点
        this.openColNode = this.widget.getWSOpenColNodeByID();      //展开所有列表头节点
        this.freezeDim = this.widget.getWSFreezeDimByID();           //冻结维度
        this.themeColor = this.widget.getWSThemeColorByID();         //主题色
        this.tableForm = this.widget.getWSTableFormByID();           //表格类型
        this.tableStyle = this.widget.getWSTableStyleByID();         //表格风格
        ...
        this.crossItems = [];
        this.mergeCols = [];
        this.columnSize = this.widget.getWSColumnSizeByID(wId);
    ...
    resetETree() {
        this.eTree = new Tree();
        this.crossETree = new Tree();
    }

    /**
     * format展开节点树
     */
    _formatExpanderTree(eTree) {
        var self = this, result = [];
        each(eTree,  (t)=> {
            var item = {};
            item.name = t.node.name;
            if (isNil(t.children)) {
                item.children = this._formatExpanderTree(t.children);
            }
            result.push(item);
        });
        return result;
    ...
    _parseClickedValue4Group(v, dId) {
        var group = this.widget.getDimensionGroupByID(dId);
        var fieldType = this.widget.getFieldTypeByDimensionID(dId);
        var clicked = v;
     ...
     * 展开所有节点的情况下的收起    横向
     */
    _addNode2eTree4OpenRowNode(nodeId) {
        var self = this;
        var clickNode = self.eTree.search(nodeId);
        if (BI.isNull(clickNode)) {
     ...
     * 展开所有节点下的收起   纵向
     */
    _addNode2crossETree4OpenColNode(nodeId) {
        var self = this;
     ...
     * 交叉表的(指标)汇总值
     */
    _createTableSumItems(s, sum, pValues, ob, isLast) {
        var self = this;
     ...
     * 初始化 crossItemsSum
     */
    _initCrossItemsSum(currentLayer, sums) {
        var self = this;
        currentLayer++;
     ...
     * 交叉表——crossItems
     */
    _createCrossPartItems(c, currentLayer, parent) {
    ...
    _createItems4OnlyCrossAndTarget() {
        var self = this;
        var l = this.data.l;
        var items = [];
        BI.each(this.targetIds, function (i, tId) {
